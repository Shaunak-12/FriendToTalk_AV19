...
@Component({
  ...
  imports: [AdvanceTitleHeadNewComponent, AdvanceTableComponent, AddAdminDetailsComponent,
...
export class AddAdminComponent implements OnInit {
  ...
  currentQuery={ "Search": "", "intParam1": "-1", "intParam2": "0","SiteCode":sessionStorage.getItem('selectedSite')};
  RolesList=[];
  ...
  getAllData()
  {
    this.apiservice.getRequest(config['getAdminRoles'],'getAdminRoles').subscribe((data: any) => {
      this.RolesList = data;
      console.log("admin getadmin roles:-", this.RolesList);
      this.RolesList.forEach(({Id, RoleName}) => {
        this.dynamicControls[1].options?.push({ val: Id, op: RoleName });
      });
      this.GetAllAdmins();
    }, (error) => {
      console.log(error);
