...
@Component({
  ...
  imports: [AdvanceTitleHeadNewComponent,AdvanceTableComponent,AdvanceTablePaginatorComponent,CompleteComponent,UpdateUserComponent,CallingPopComponent,
...
export class CallLeadComponent implements OnInit {
  ...
  assignList: any = [];
  pagesTotal = 1;
  udataToView: { Mobile?: any; CountryCode?: any; tempMobile?: any; } = {};
  AcceptRejectVar = "A";
  paginatorBlock: any = [];
  ...
  UserDataCollumns: { value: any; bg: any; }[][] = [];
          ...
          this.CallRequestinfoData.push([
            ...
            { value: element.AssignAdmin, bg: bg_cell },
            {
              bg: bg_cell, icon: 'Multi', value: [
                ...
                // ...([{ value: 'Call2', bg: bg_cell, icon: 'None' }]),
              ]
            },
  ...
  CallOption2() {
    let option = "?phone=" + this.udataToView['Mobile']+'&CountryCode='+this.udataToView['CountryCode'];
    this.apiservice.getRequest(config['callLead1'] + option, 'callLead1').subscribe({
      next: (data:any) => {
        if(data.ErrorCode == '1'){
