...
@Component({
  ...
  imports: [AdvanceTitleHeadNewComponent, CardListComponent, AdvanceTableComponent, SharedModule, CommonModule],
...
export class MyDashboardComponent implements OnInit, OnDestroy {
  ...
  GetDashboardChart() {
    ...
    this.apiSubscriber[1] = this.apiservice.sendRequest(config['getDashboardDepositChart'], this.currentQuery2, 'DashboardChart').subscribe((data: any) => {
      ...
      var withdrawal: any = [];
      var reverse: any = [];
      let startDate = this.datePipe.transform(this.currentQuery2.StartDateTime, 'MM-dd-yyyy') || '';
      let endDate = this.datePipe.transform(this.currentQuery2.EndDateTime, 'MM-dd-yyyy') || '';
      let label = this.utilities.getDatesInRange(new Date(startDate), new Date(endDate));
