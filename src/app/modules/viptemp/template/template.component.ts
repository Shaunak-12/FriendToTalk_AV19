...
@Component({
  ...
  imports: [MultiInputHeaderComponent,AdvanceTableComponent,AddFormTempComponent,
...
export class TemplateComponent implements OnInit {
  ...
  defGrp:any=[];
  dynamicControls = [
      {changeAction:'submit',type:'select',default:{name:this.defWal.Name+'-'+this.defWal.Code,value:this.defWal.Id},options:this.userWals.filter(({Id}: {Id: number})=>Id!==this.defWal.Id).map(({Id,Name,Code}: {Id: number, Name: string, Code: string})=>({name:Name+'-'+Code,value:Id}))},
  ...
  selWal(formVal:any)
  {
    ...
    if(mGroup&&mGroup.WalletwsList[0]){
      this.defGrp=mGroup.WalletwsList.map(({ Id, Name }: { Id: number, Name: string }) => ({ name: Name, value: Id }));
      console.log(this.defGrp[0]);
      this.dynamicControls[1].default=this.defGrp[0];
      this.dynamicControls[1].options=this.defGrp.filter(({value}: {value: number})=>value!==this.defGrp[0].value);
