...
@Component({
  ...
  imports: [MultiInputHeaderComponent,AdvanceTableComponent,AddFormTfcComponent,
...
export class TempfeatureconfigComponent implements OnInit {
  ...
  defWal=this.userWals.filter((wal: { Id: number; Name: string; Code: string; }) => wal.Id === parseInt(sessionStorage.getItem('WalChosen') || '{}') as number)[0] as { Id: number; Name: string; Code: string; };
  allTmp:any=[];
  allLev:any=[];
  allFeatures:any=[];
  defTmp:any=[];
  ...
  ngOnInit(): void {
    ...
    this.apiSubscriber[1] = this.apiservice.getRequest(config['getVIPTmpFeatures'], 'getVIPTmpFeatures').subscribe((data: any) => {
      this.allFeatures=data.map((item: { Id: any; Name: any }) => ({ name: item.Name, value: item.Id }));
  ...
  selWal(formVal:any)
  {
    let mTmp = this.allTmp.find((wal: { WalletTypeId: number }) => wal.WalletTypeId == formVal.C0);
