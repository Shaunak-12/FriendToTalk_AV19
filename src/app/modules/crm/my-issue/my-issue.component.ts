...
@Component({
  ...
  imports: [AdvanceTitleHeadNewComponent,AdvanceTableComponent,AdvanceTablePaginatorComponent,AddCommentsComponent,
  ...
  intializeData() {
    ...
    // if (this.priority && this.priority.length > 0) {
      this.dynamicControls = [
        { changeAction: 'submit', que: 'priority', type: 'dropdown', default: '', options: [{ op: 'All Priorities', val: '' }, ...this.priority.map((value: { name: any; code: any; }) => ({ op: value.name, val: value.code }))], subque: [] },
        { changeAction: 'submit', que: 'status', type: 'dropdown', default: '', options: [{ op: 'All Status', val: '' }, ...this.status.map((value: { name: any; code: any; }) => ({ op: value.name, val: value.code }))], subque: [] },
        { changeAction: 'submit', que: 'category', type: 'dropdown', default: '', options: [{ op: 'All Categories', val: '' }, ...this.category.map((value: { name: any; code: any; }) => ({ op: value.name, val: value.code }))], subque: [] },
