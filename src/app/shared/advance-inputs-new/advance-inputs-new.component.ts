...
export class AdvanceInputsNewComponent implements OnInit {
  @Input() question: any;
  ...
  ngOnInit(): void {
    if (this.question?.changeAction === 'submit') {
      if (this.question?.type !== 'daterange') {
        this.formGroup.get('value')?.valueChanges.subscribe(() => {
          this.submitTrigger.emit();
        });
      } else {
        this.formGroup.get('value1')?.valueChanges.subscribe(() => {
          this.submitTrigger.emit();
        });
        this.formGroup.get('value2')?.valueChanges.subscribe(() => {
          this.submitTrigger.emit();
        });
      }
    }
